...
<div class="h-full flex flex-col bg-white dark:bg-gray-900">
  <div class="p-3 border-b border-gray-200 dark:border-gray-700">
    <h2 class="text-sm font-semibold">{$t('discovery.relatedPages')}</h2>
  </div>

  <div class="flex-1 overflow-y-auto p-2 space-y-1">
    {#if discoveryStore.relatedPages.length === 0}
      <p class="text-xs text-gray-400 px-2 py-4">{$t('discovery.noRelated')}</p>
    {:else}
      {#each discoveryStore.relatedPages as page (page.id)}
        ...
        >
          <div class="flex items-center gap-2">
            ...
            <span class="text-sm font-medium truncate flex-1">{page.title || $t('editor.titlePlaceholder')}</span>
            <span class="text-xs text-gray-400">{page.score}</span>
          </div>
          {#if page.reasons.length > 0}
            <div class="flex gap-1 mt-1 flex-wrap">
              {#each page.reasons as reason}
                <span class="text-xs px-1.5 py-0.5 rounded bg-gray-100 dark:bg-gray-700 text-gray-500">{reason}</span>
              {/each}
            </div>
          {/if}
        </button>
