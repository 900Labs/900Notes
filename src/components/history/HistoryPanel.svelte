...
<div class="h-full flex flex-col bg-white dark:bg-gray-900">
  ...
  <div class="flex-1 overflow-y-auto p-2 space-y-1">
    {#if historyStore.revisions.length === 0}
      <p class="text-xs text-gray-400 px-2 py-4">{$t('history.empty')}</p>
    {:else}
      {#each historyStore.revisions as revision (revision.id)}
        <div
          class="p-2 rounded-lg border cursor-pointer transition-colors {selectedRevisionId === revision.id ? 'border-accent bg-accent/5' : 'border-gray-200 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-800'}"
        ...
        >
          <div class="flex items-center justify-between mb-1">
            <span class="text-xs text-gray-500">{formatDate(revision.createdAt)}</span>
          ...
          <p class="text-sm font-medium truncate">{revision.title || $t('editor.titlePlaceholder')}</p>
          {#if selectedRevisionId === revision.id}
            <p class="text-xs text-gray-500 mt-1 line-clamp-4">{getPreview(revision.content)}</p>
          {/if}
        </div>
      {/each}
    {/if}
